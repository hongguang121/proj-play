...
$(document).ready(function() {
    ...
    //滚动监听
    var scroll_listener = setInterval(function(){
    ...
    ////////////////////////////////////////////////////////////////////////////////
        ...
        $(".image-upload").change(function () {
            ...
            change_high();
                ...
                $.post("https://pic.mouchina.com/image/upload",
                    function(img_url){
                        console.log(img_url);
                    });
            }
                ...
                $(img_upload_name).change(function () {
                    if (curr_pic_cnt < 5) {
                        ...
                        $.post("https://pic.mouchina.com/image/upload",
                            function(img_url){
                                console.log(img_url);
                                //GET https://teststatic.mouchina.com/static/H5-3.0/poem/img/pic/C:%C3%BAkepathindex1.jpg 404 (Not Found)
                            });
                    }
                });
                change_high();
    ...
    $("#final").click(function(){
        ...
        //evt_lst:预览或取回数据的顺序链
        var send_json = {title: "诗与远方测试", preferredPic: "http://localhost", labelType: 0, contents: [evt_lst, remb_text.text, remb_pic.images, remb_good.goods]};
        //alert(send_json);
        //console.log(JSON.stringify(send_json));
        //var res = JSON.parse(send_json);
        //var send = "{title: \"诗与远方测试\", preferredPic: \"http://localhost\", labelType: 0, contents:[{abc: 123}]}";
        //XMLHttpRequest cannot load https://pic.mouchina.com/image/upload. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'https://teststatic.mouchina.com' is therefore not allowed access. The response had HTTP status code 504.

        $.ajax({
            url: "https://test1.mouchina.com/poem/new?loginKey=d8cbf6e0f759b481e2abbff9b28a9b912af4c400",
            type: "POST",
            dataType: "jsonp",
            jsonp: "jsoncallback",
            data: send_json,
