...
(define wlist (file->string "e:/source/words/wlist.txt"))        ;我的单词表
(define slist (file->string "e:/source/words/slist.txt"))        ;我的词组表
(define vocab (file->list "e:/source/words/vocabulary.txt"))     ;6000词频表
(define v15000 (file->string "e:/source/words/v15000.txt"))      ;15000词频表
;(define gre (file->string "e:/source/words/gre.txt"))            ;GRE词汇
;(define simple (file->list "e:/source/words/simple.txt"))        ;简单词表
(define idlist (file->string "e:/source/words/identical.txt"))   ;特殊变形表
(define org (file->list "e:/source/words/origin.txt"))           ;动词原形表
(define dict+ (file->lines "e:/source/words/dict.txt"))          ;牛津英汉
(define han (file->string "e:/source/words/han.txt"))            ;汉字
(define han-word (file->string "e:/source/words/han-word.txt"))  ;汉语词组
(define article (file->string "e:/source/words/test.txt"))       ;文本
(define nov2666 (file->string "e:/source/words/2666.txt"))
(define novgwtw (file->string "e:/source/words/gone_with_the_wind.txt"))
(define moby_dick (file->string "e:/source/words/melville-moby_dick.txt"))
(define nov17p1 (file->string "e:/source/words/虚拟的十七岁上.txt"))
(define nov17p2 (file->string "e:/source/words/虚拟的十七岁下.txt"))
...
(define (format* ls sym)
  ...
  (define (fstr str cnt line check rem res)
    (cond ((= line 2)
          ...
          ((string=? (to-string (string-ref str cnt)) "\r")
           (fstr str (+ cnt 1) line "" (append rem (list check)) res))
          ((string=? (to-string (string-ref str cnt)) "\n")
           (fstr str (+ cnt 1) (+ line 1) "" rem res))          
          (else (fstr str (+ cnt 1) line (string-append check (to-string (string-ref str cnt))) rem res))))
  (define (idstr str cnt check rem res)
    (cond ((= (string-length str) cnt) (reverse res))
          ...
          (else (idstr str (+ cnt 1) (string-append check (to-string (string-ref str cnt))) rem res))))
  (select))
...
;排序mywords
;(set! mywords (sort mywords string<=? #:key car))
...
;str为单词字符串，支持模糊查询
(define (find-en str)
  (filter (λ (x) (or (string-contains? (car x) str)
                     (string-contains? str (car x)))) mywords))
  ...
  (define (iter sign cnt rem)
    (cond ((= (string-length str) cnt) rem)
          ((and (< (+ cnt 1) (string-length str))  ;跳过"."后面的空格和","(根据情况)
                (string=? (to-string (string-ref str cnt)) ".")
                (or (string=? (to-string (string-ref str (+ cnt 1))) " ")
                    (string=? (to-string (string-ref str (+ cnt 1))) ",")))
           (iter sign (+ cnt 2) (string-append rem (to-string (string-ref str cnt)))))          
          ((and (>= cnt 1)
                (eq? #f (not (member (to-string (string-ref str cnt)) args)))
                (not (string=? (to-string (string-ref str (- cnt 1))) "."))  ;如果需要过滤的符号前是"."，则不在它之后添加空格                
                (not (string=? (to-string (string-ref rem (- (string-length rem) 1))) " "))  ;如果已经有了空格则不再添加
                (= sign 4))
           (iter 0 (+ cnt 1) (string-append rem " ")))
          ((eq? #f (not (member (to-string (string-ref str cnt)) args)))                
           (iter 0 (+ cnt 1) rem))
          (else (iter 4 (+ cnt 1) (string-append rem (to-string (string-ref str cnt)))))))
  (iter 4 0 ""))
...
(define (draw str)
  (define (iter cnt check rem)
    (cond ((= (string-length str) cnt) (reverse (cons check rem)))          
          ...
          (else (iter (+ cnt 1) (string-append check (to-string (string-ref str cnt))) rem))))
  (iter 0 "" '()))
...
(define (mkset str)
  ...
  (iter (sort (map string-downcase (draw str)) string<?) "" 1 '()))
  ...
  (let* ((ch (string-ref word 0))
    ...
    (let loop ((id start))               
      (cond ((= id end) '("not found" "not found"))
            ((regexp-match (regexp (string-append "^" word)) (list-ref dict+ id))
             (cons word (list (list-ref dict+ (+ id 1)))))
            (else (loop (add1 id)))))))
;  (let ((res (member word dict+ (λ (x y) (regexp-match (regexp (string-append "^" x)) y)))))
...
(define mydb (mysql-connect #:user USER
                            #:password PASS
                            #:database "test"))
